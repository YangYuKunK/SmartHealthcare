                ...
                <el-form :model="loginForm" :rules="rules" ref="loginForm" class="demo-ruleForm">
                    ...
                    <el-form-item>
                        <el-button type="primary" size="medium" :loading="loading" style="width:100%"
                            @click="submitForm('loginForm')">立即登陆</el-button>
                        <!-- <el-button type="primary1" size="medium1" :addding="addding" style="width:100%"
                            @click="loginEnroll">注册账户</el-button> -->
                    </el-form-item>
                </el-form>
            </div>
        </div>
...
export default {
    ...
    methods: {
        submitForm(loginForm) {
            Axios({
                url: 'https://localhost:44388/api/User/SelectUserInfo?admin=' + this.loginForm.username + '&pass=' + this.loginForm.password,
                method: 'get'
                // headers: {
                //     authorization: window.sessionStorage.getItem("token")//从本地取出token
                // }
            }).then(ex => {
                // console.log(ex.data)
                // console.log(ex.data.data.usersId > 0 ? "1" : "0")
                if (ex.data.code == 200) {
                    console.log(ex)
                    this.$message({
                        message: '恭喜你，用户登录成功！',
                    ...
                    // console.log(window.sessionStorage.getItem("token"))//将token存储在session中
                    this.$store.commit("set_token", ex.data.jwttoken)//将token存入vuex
                    this.loading = true
                    this.login(ex)
                } else {
                    this.$message.error('很遗憾，用户登录失败！');
                    return false;

                }
            })

            // //例子(将token字符串取出后加入请求头)
            // Axios({
            //   url: 'https://localhost:44322/api/Goods/GetGoods',
            //   method: 'get',
            //   headers: {
            //     authorization: window.sessionStorage.getItem("token")//从本地取出token
            //   }
        ...
        login() {
            this.$store
                .dispatch('user/login', { username: 'admin' })//将用户名传至验证组件
                .then(() => {
